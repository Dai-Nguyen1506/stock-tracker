...
// ── Create Mock History ────────────────────────
const generateMockHistory = (limit: number, endTimestampMs: number, intervalStr: string, startPrice = 60000) => {
  let intervalMs = 60000; // Mặc định 1 phút
  if (intervalStr === '5m') intervalMs = 5 * 60000;
  if (intervalStr === '15m') intervalMs = 15 * 60000;
  if (intervalStr === '1h') intervalMs = 60 * 60000;
  if (intervalStr === '4h') intervalMs = 4 * 60 * 60000;
  if (intervalStr === '1d') intervalMs = 24 * 60 * 60000;

  let currentPrice = startPrice;
  const mockData = [];
  
  let currentTimestamp = endTimestampMs - (limit * intervalMs);

  for (let i = 0; i < limit; i++) {
    const volatility = (Math.random() - 0.5) * (startPrice * 0.005);
...
export const MainChart: React.FC<MainChartProps> = ({ selectedSymbol, selectedInterval, setSelectedInterval }) => {
  ...
  const loadHistory = useCallback(async (sym: string, iv: string, limit: number, prepend = false) => {
    ...
    try {
      ...
      if (prepend && oldestTsRef.current > 0) {
        endTimestamp = oldestTsRef.current; 
        if (currentDataRef.current.candles.length > 0) {
            startPrice = currentDataRef.current.candles[0].open;
        }
      }

      // Create mock data instead of calling real API
      const mockHistory = generateMockHistory(limit, endTimestamp, iv, startPrice);
      const sorted = [...mockHistory].sort((a: any, b: any) => a.timestamp - b.timestamp);
  ...
  const kline = useBinanceKlineStream(selectedSymbol); 
